...
4. **Find public interview discussion.** Extract the JD role title first, then query the `PublicDiscussionSearch` adapter with at most two role-aware, company-focused queries. Boost relevant discussion platforms without restricting the broader web, rank diverse public results, safely fetch/clean allowed pages, and retain only Tavily's bounded source-attributed excerpt when direct retrieval is unavailable. Reserve prompt space for public discussion and treat it only as candidate-reported format context. Search/no-result/provider errors become recorded warnings, not a failed kit.
