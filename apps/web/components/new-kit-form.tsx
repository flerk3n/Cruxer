...
export function NewKitForm() {
  ...
  return <><div className="relative overflow-hidden rounded-[2rem] border bg-surface/75 px-6 py-7 shadow-[0_14px_45px_hsl(var(--ink)/0.06)] sm:px-8"><div className="workspace-grid pointer-events-none absolute inset-0 opacity-40" aria-hidden="true" /><div className="relative"><p className="workspace-kicker text-xs">Your next role</p><h1 className="mt-2 text-[clamp(2rem,4vw,3.25rem)] font-semibold tracking-[-0.05em]">New preparation kit</h1><p className="mt-2 text-sm text-muted-ink">Paste the posting. Cruxer will do the reading around it.</p></div></div>
    ...
    </div><aside className="border-t bg-ink/[0.025] p-5 sm:p-8 lg:border-l lg:border-t-0"><FieldLabel htmlFor="days">Interview window</FieldLabel><div className="flex items-center gap-3"><Input id="days" name="days" type="number" min="1" max="60" defaultValue="5" className="w-24" disabled={busy} /><span className="text-sm text-muted-ink">days</span></div><GenerationStatus run={run} loading={state === "starting" || state === "polling"} onRetry={state === "failed" && run?.status === "retryable" ? () => void retryRun() : undefined} />
