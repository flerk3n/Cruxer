...
- Hardened Gemini generation for the free tier: category calls now pass through a configurable serial request gate (4.5 seconds by default), explicit Gemini 429 retry delays are preserved, and a malformed structured response receives one schema-aware repair prompt before failing cleanly. Added focused tests for pacing, provider delay handling, and bounded repair. The clean live five-case Appendix B run remains pending Gemini quota recovery; local verification is complete.
