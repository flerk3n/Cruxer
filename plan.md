...
| LLM | Gemini Flash model selected from the currently available free-tier models, via `@google/genai` structured JSON mode | Google documents a free tier and structured-output JSON Schema support. Zod still validates every result because provider-conformant JSON is not semantic validation. |
