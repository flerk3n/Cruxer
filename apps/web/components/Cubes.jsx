...
const Cubes = ({
  ...
  const getCubes = useCallback(() => {
    if (!cubesRef.current.length && sceneRef.current) {
      cubesRef.current = Array.from(sceneRef.current.querySelectorAll('.cube'));
    }
    return cubesRef.current;
  }, []);

  const tiltAt = useCallback(
    (rowCenter, colCenter) => {
      if (!sceneRef.current) return;
      const nextActiveCubes = new Set();
      ...
      activeCubesRef.current = nextActiveCubes;
    },
    [getCubes, radius, maxAngle, enterDur, leaveDur, easing]
  );
  ...
  const onClick = useCallback(
    ...
    [getCubes, rippleOnClick, gridSize, faceColor, rippleColor, rippleSpeed]
  );
